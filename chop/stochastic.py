...
class PGD(Optimizer):
    ...
    def __init__(
        self, params, prox=None, lr=0.1, momentum=0.9, normalization="none"
    ):
        if prox is None:
        ...
        for prox_el in prox:
            if prox_el is not None:
                self.prox.append(
                    lambda x, s=None: prox_el(x.unsqueeze(0)).squeeze()
                )
            else:
        ...
        for groups in self.param_groups:
            for p in groups["params"]:
                ...
                state["grad_estimate"].add_(
                    grad - state["grad_estimate"], alpha=1.0 - self.momentum
                )

                grad_est = normalize_gradient(
                    state["grad_estimate"], self.normalization
                )
                ...
                new_p = self.prox[idx](p - step_size * grad_est, 1.0)
...
class S3CM(Optimizer):
    ...
    def __init__(
        self, params, prox1=None, prox2=None, lr=0.1, normalization="none"
    ):
        ...
        for prox1_, prox2_ in zip(prox1, prox2):
            ...
            self.prox1.append(
                lambda x, s=None: prox1_(x.unsqueeze(0), s).squeeze(dim=0)
            )
            self.prox2.append(
                lambda x, s=None: prox2_(x.unsqueeze(0), s).squeeze(dim=0)
            )
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    state["dual"] = (
                        state["iterate_1"] - state["iterate_2"]
                    ) / self.lr
                ...
                state["iterate_1"] = self.prox1[idx](
                    state["iterate_2"] - self.lr * (grad + state["dual"]),
                    self.lr,
                )
...
class PairwiseFrankWolfe(Optimizer):
    ...
    def __init__(self, params, lmo_pairwise, lr=0.1, momentum=0.9):
        if not (type(lr) == float or lr == "sublinear"):
            raise ValueError("lr must be float or 'sublinear'.")
        ...
        self.momentum = momentum
        defaults = dict(
            lmo=self.lmo, name=self.name, lr=self.lr, momentum=self.momentum
        )
    ...
    def __init__(
        ...
        for param, oracle in zip(params, lmo):
            if oracle:
                ...
                msg = (
                    f"No LMO was provided for parameter {param}. "
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                state["grad_estimate"].add_(
                    grad - state["grad_estimate"], alpha=1.0 - momentum
                )
                update_direction, _ = self.lmo[idx](-state["grad_estimate"], p)
                state["certificate"] = (
                    -state["grad_estimate"] * update_direction
                ).sum()
